...
export const EventUtil = {
    addHandler: function(element: any, type: string, handler: any, pop = false) {
    ...
    removeHandler: function(element: any, type: string, handler: any, pop = false) {
 ...
 * 身份证，手机号码加*号处理
 */
export function starParse(str: string, isPhone: any) {
    if (!str) {return str}
...
// 设定数字校验区间
export const checkNumberRange = (min: number, max: number) => {
    return (rule: any, value: unknown, callback: (arg0: Error | undefined) => void) => {
...
/* 检测是否有键 */
export function hasKey(target: any, propName: string) {
...
export function sign(params: any):any {
    let str = ''
...
export function checkEmpty(val: string | null | undefined):boolean {
